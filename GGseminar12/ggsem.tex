...
\small
\begin{itemize}
\item so now we are \alert{ice sheet modellers}, the chosen few \dots
...
\item instead of ``eyeballing'' we can \emph{measure} errors from the numerical code
\end{itemize}
\end{column}
...
\small
\begin{itemize}
\item from now on, let's assume we have built an ``ice sheet model'' and have already spent some time verifying it
\item i.e.~let's skip over \emph{my} career, as some kind of minor detail \dots
\end{itemize}
\end{frame}
...
\begin{itemize}
\item FIXME:  figure from Habermann?
\end{itemize}
\end{frame}
