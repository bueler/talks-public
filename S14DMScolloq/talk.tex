...
\vspace{-4mm}
#2{\item a dot-product form for projection to closed, convex $K\subseteq \mathcal{H}$:
   $$y=\Pi x: \qquad (y - x) \cdot (\eta - y) \ge 0 \,\, \text{ for all } \eta \in K$$}
\vspace{-4mm}
#3{\item a rewriting of 11min $f$ on convex $K\subseteq \RR^n$'':
...
\item let $f: K \to \RR$ be smooth ($C^1$)
...
\uncover<3->{which is a \alert{variational inequality}}
\end{column}
