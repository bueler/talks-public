...
Proof. $x' \mapsto \Pi (x'-F(x'))$, as map on $K$, has a fixed point.
\begin{center}
...
if $K$ is convex and $u,v\in K$ and $0\le \varepsilon \le 1$ then
    $$w = (1 - \eps) u + \eps v = u + \eps (v-u) \in \mathcal{K}$$

\begin{columns}
\begin{column}{0.5\textwidth}
\includegraphics[width=1.0\textwidth]{convexview-lincomb}

$w = (1 - \eps) u + \eps v$ pictured as a linear combination in $K$
\end{column}
\begin{column}{0.5\textwidth}
\includegraphics[width=1.0\textwidth]{convexview-intoK}

$w = u + \eps (v-u)$ pictured as a vector from $u$ directed into $K$
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}
  \frametitle{ice sheet geometry: an analogy}
...
\item where it snows some places \dots
\item and melts others
\item like North America in 90,000--20,000 b.c.e.
\item question: what land is covered by ice sheets?
\item<2-> answer: Ontario, mostly.  Illinois, mostly not.
\end{itemize}
...
\item Theorem (Schoof, 2006).  There is unique velocity $\mathbf{U}=(u,v)\in W^{1,q}(\Omega)$ solving the \emph{coulomb ice stream problem}.  It minimizes
\small
\begin{equation*}
...
\begin{itemize}
\item Schoof first derived the variational inequality form: $\mathbf{U} \in W^{1,q}(\Omega)$ solves
\begin{align*}
...
\begin{itemize}
\item of the three v.i.s:
  \begin{itemize}
  \item[$\circ$] 1 for the ice sheet surface is \emph{not} a minimization
  \item[$\circ$] 2 for the ice stream sliding \emph{is} a minimization, but of a non-smooth functional
  \item[$\circ$] 3 for the cold ice is \emph{not} a minimization
  \end{itemize}
\item<2-> v.i.s will be used a lot in the future for glacier and ice sheet problems because of all the free boundaries between different equations
  \begin{itemize}
  \item<2->[$\circ$] I've learned that when a glaciologist says ``this equation describes this \dots'' they mean ``this equation describes this \dots \emph{whereever the equation can be applied and I can't generally tell you where that is}''
  \item<2->[$\circ$] this makes the job of building an ice sheet model hard
  \end{itemize}
