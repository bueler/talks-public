...
\vspace{-4mm}
#2{\item a dot-product method for projection onto convex $K\subseteq \mathcal{H}$:
   $$y=\Pi_K x: \qquad (y - x) \cdot (\eta - y) \ge 0 \,\, \text{ for all } \eta \in K$$}
\vspace{-4mm}
#3{\item rewrite minimization; min $f$ on convex $K\subseteq \RR^n$ solves:
   $$u: \qquad \grad f (u) \cdot (v - u) \ge 0 \qquad \text{ for all } v \in K$$}
\vspace{-4mm}
#4{\item solve any nonlinear eqns on compact, convex $K_0\subset \RR^n$:
...
\section[problems]{mathematical problems that you can write as variational inequalities}
...
% where we stand
\whatvi{\visible}{\invisible}{\invisible}{\invisible}
...
\begin{frame}
  \frametitle{FIXME: min $f$ on convex $K\subset \RR^n$}
...
\section[glaciers]{three applications to glacier problems}
