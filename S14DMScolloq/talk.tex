...
\begin{itemize}
\item<1-> suppose you have a smooth function
\item<2> \emph{on a closed, bounded interval}
\item<1-> and you want to minimize it
\end{itemize}
\end{frame}
...
\begin{center}
\includegraphics[height=0.4\textheight]{calcprob_soln_interior.png}
\end{center}
\end{column}
\begin{column}{0.5\textwidth}
\begin{center}
\includegraphics[height=0.4\textheight]{calcprob_soln_endpoint.png}
\end{center}
\end{column}
\end{columns}
...
because it is smooth, you can say about $u$ that:
\only<1>{
\vspace{1mm}

\begin{itemize}
\item if $a < u < b$ then $f'(u)=0$, or
\item if $u = a$ then $f'(u) \ge 0$, or \hfill [$\longleftarrow$ \emph{not shown}]
