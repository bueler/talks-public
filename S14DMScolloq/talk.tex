...
% this slide reappears
\newcommand{\whatvi}[5]{
\begin{frame}
  \frametitle{what is a \emph{variational inequality}?}

\begin{enumerate}
#1{\item a cute way to rewrite a calc problem:
   $$u: \qquad f'(u) (v - u) \ge 0 \qquad \text{ for all } v \in [a,b]$$}
\vspace{-5mm}
#2{\item a dot-product way to describe projection onto $K\subseteq \mathcal{H}$:
   $$y=\Pi_K x: \qquad (y - x) \cdot (\eta - y) \ge 0 \,\, \text{ for all } \eta \in K$$}
\vspace{-5mm}
#3{\item any horrible, nonlinear system of equation has a solution:
   $$FIXME: \qquad (y - x) \cdot (\eta - y) \ge 0 \,\, \text{ for all } \eta \in K$$}
\vspace{-5mm}
#4{\item rewrite constrained minimization (e.g.~obstacle) problems:
   $$\int_0^L u' (v'-u') - f (v-u) \ge 0 \qquad \forall v\in\mathcal{K}$$}
\vspace{-5mm}
#5{\item a well-posed problem when there is a free boundary:}
\end{enumerate}
\end{frame}
}
...
suppose you have a smooth function \only<2>{\emph{on a closed, bounded interval }}and you want to minimize it
\end{frame}
...
the single \alert{variational inequality} applies,
    $$f'(u) (v - u) \ge 0 \qquad \text{ for all } v \in [a,b]$$
}
\end{frame}

% where we stand; skip the first
%\whatvi{\visible}{\invisible}{\invisible}{\invisible}{\invisible}

\begin{frame}
  \frametitle{projection onto a closed, convex set $K$}

\begin{center}
\includegraphics[height=0.4\textheight]{proj_xy.png}
\end{center}

Suppose $K \subseteq \RR^n$ (or $K \subseteq \mathcal{H}$ in a Hilbert space) is closed and convex.
\medskip

Definition.  If $x\in\RR^n$ (or $x \in \mathcal{H}$) then the unique minimizer
    $$y = \min_{z\in K} \|x-z\|$$
is the \emph{projection} of $x$ onto $K$, written $y = \Pi_K x$.
\end{frame}
...
Theorem.
   $$y = \Pi_K x \qquad \iff \qquad (y - x) \cdot (\eta - y) \ge 0 \,\, \text{ for all } \eta \in K$$

\medskip
This is also a \alert{variational inequality}.

\medskip
Idea.  The angle between $y-x$ and $\eta - y$ is $\le 90^\circ$ for all $\eta \in K$.
\end{frame}


% where we stand
\whatvi{\visible}{\visible}{\invisible}{\invisible}{\invisible}
...
% where we stand
\whatvi{\visible}{\visible}{\visible}{\visible}{\invisible}
