...
\item questions:
  ...
  \item[$\circ$] are implicit time-steps well-posed?
    \begin{itemize}
    \item[$\triangleright$] explicit time steps are well-posed, but give severe stability time-step restrictions
    \end{itemize}
  \item[$\circ$] can a time-stepping model exactly balance change in fluid volume over time step by computable integrals of $f$?
  \end{itemize}
\end{itemize}
\begin{center}
\includegraphics[width=0.55\textwidth,keepaspectratio=true]{cartoon-sets-crop}
\end{center}
\end{frame}


\begin{frame}{weak form of an implicit time step}
FIXME
...
\begin{center}
\includegraphics[width=0.6\textwidth,keepaspectratio=true]{cartoon-sets} \hfill \includegraphics[width=0.4\textwidth,keepaspectratio=true]{domains-fig}
\end{center}
