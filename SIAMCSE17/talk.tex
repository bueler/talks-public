...
\item Greenland-sized radial ice sheet
    ...
    \end{itemize}
\item<1> reduced-space Newton step has Jacobian in inactive variables only
\item<2> \dots thus on fine grids ($\lesssim 1$ km) the number of Newton iterations is proportional to margin advance divided by grid spacing
...
\begin{center}
\includegraphics[width=0.7\textwidth]{startsheet.png}

\includegraphics[width=0.7\textwidth]{endsheet.png}
\end{center}

\begin{itemize}
\item solves: $H_t + \Div \mathbf{q} = m$ where $\mathbf{q} = -D\grad H + VH$ and $m=m(s)$
    \begin{itemize}
    \item[$\circ$] imposed $V(x,y)$
    \item[$\circ$] elevation-dependent accumulation/ablation
    \end{itemize}
\item initial state (upper) is just snow piled up (shows ela)
\item method: ARKIMEX + $3$ or more NCP solves at each time step
\end{itemize}
\end{frame}
