...
\item one needs a stack of four levels of tools:
    ...
    \vspace{-1mm}
    \begin{itemize}
    \item[$\circ$] commercial packages may hide this stack
    \end{itemize}
\item the two stacks I have used
    \begin{enumerate}
    \item Gmsh $\to$ (my own C code) $\to$ PETSc $\to$ (my own tools)
    \item Gmsh $\to$ Firedrake [Python] $\to$ PETSc $\to$ Paraview
    \end{enumerate}
\item I will show examples of each
\end{itemize}
\end{frame}
...
\item different meshers have different quality:
...
\item \texttt{unfem.c} uses PETSc (\href{https://www.mcs.anl.gov/petsc/}{\color{blue} \texttt{www.mcs.anl.gov/petsc}}) to assemble the FEM residual
\item \dots then it calls a nonlinear solver (SNES) in PETSc
\end{itemize}

this run uses algebraic multigrid-preconditioned CG on the level-6 fractal domain:
\begin{cline}
$ domain.py -l 6 -o koch6.geo
$ gmsh -2 koch6.geo
$ msh2petsc.py koch6
$ unfem -un_mesh koch6 -un_case 4 \
...
\begin{frame}[fragile]
\frametitle{Unified Form Language example}

\begin{itemize}
\item UFL is \emph{expressive} language for weak forms
\item consider problem $-\triangle u = f$ with $u=g$ on $\partial\Omega$, and weak form
    $$\int_\Omega \grad u \cdot \grad v - f v = 0$$
\item Python/UFL code:
\end{itemize}
\begin{code}
Vh = FunctionSpace(mesh,'CG',degree=k)      # V_h = P^k
u = Function(Vh)
v = TestFunction(Vh)
F = (dot(grad(u), grad(v)) - f * v) * dx    # weak form
bc = DirichletBC(Vh, g, bdry_ids)
solve(F == 0, u, bcs=[bc], ...)
\end{code}
\end{frame}
