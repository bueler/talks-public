...
\medskip
\item PDE solution by algebraic multigrid-preconditioned CG on the level-6 fractal domain:
\end{itemize}
...
\end{cline}
%$
\item $p$-refinement chooses $P^{\text{\texttt{K}}}$ on each element:
\begin{cline}
$ ./fish.py -k K
\end{cline}
%$
...
\item by error-vs-flops (left figure)
    \begin{itemize}
    \item[$\circ$] $h$-refinement runs use CG plus multigrid preconditioning
    \item[$\circ$] $p$-refinement runs use Cholesky
    \end{itemize}
\item by number of nonzeros in matrix (right figure)
