...
\item one needs 4 levels of tools:
    $$\begin{matrix} \text{mesh generation} \\ \downarrow \\ \text{FEM assembly$^1$} \\ \downarrow \\ \text{solver library$^2$} \\ \downarrow \\ \text{visualization} \end{matrix}$$
\item commercial packages may partially hide this structure, but libraries always separate these levels
\item notes:
    \begin{enumerate}
    \item includes weak form implementation, element definitions, and actual assembly
    \item includes both nonlinear solvers (e.g.~Newton method) and linear solvers (e.g.~CG and preconditioners too)
    \end{enumerate}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item the two stacks {\color{Magenta} I have actually used}:
    ...
    \qquad {\color{Magenta} \begin{matrix} \text{1.} \\ \text{Gmsh} \\ \downarrow \\ \text{my own C code} \\ \downarrow \\ \text{PETSc \scriptsize [from C] \normalsize} \\ \downarrow \\ \text{my own tools} \end{matrix}}
    \quad {\color{Magenta} \begin{matrix} \text{2.} \\ \text{Gmsh} \\ \downarrow \\ \text{Firedrake} \\ \downarrow \\ \text{PETSc \scriptsize [through Firedrake] \normalsize} \\ \downarrow \\ \text{Paraview} \end{matrix}}$$
\item I will show examples using {\color{Magenta} 1.} and {\color{Magenta} 2.}
\end{itemize}
\end{frame}
...
\item even though I used PETSc, \texttt{unfem.c} was 900 lines of C code
\end{itemize}
\begin{scode}
...
\item UFL is an \emph{expressive} language for weak forms
    ...
    \end{itemize}
\item consider Poisson problem $-\triangle u = f$ on $\Omega$ with $u=g$ on $\partial\Omega$, function space $S_0^h = P^1$, and weak form
