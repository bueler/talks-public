...
\item numbering for the example above:
    \begin{itemize}
    ...
    \end{itemize}
\item need node locations $\bx_i=(x_i,y_i)$
\end{itemize}
\end{frame}
...
\begin{itemize}
\item hat functions satisfy (classical $P_1$):
    \begin{itemize}
...
\item $\dim(S_{0}^h)=\dim(S_{g}^h)=N-n_D$
    \begin{itemize}
    \item[$\circ$] so we can assemble a square system
    \end{itemize}
...
\vspace{-2mm}
\item for $\bx_i \notin \partial_D\Omega$, (DWF) becomes:
\begin{equation*}
F_i(\bu) = \sum_{k=0}^{K-1} F_i^k(\bu) - \sum_{\nu=0}^{P-1} \varphi_i^\nu = 0
\end{equation*}
\item Dirichlet boundary nodes are degrees of freedom (easier!):
\begin{equation*}
...
\begin{itemize}
\item result of assembly is nonlinear system:
\begin{equation*}
\bF(\bu)=0  \tag{$\ast$}
\end{equation*}
\item \emph{residual} $\bF:\RR^N \to \RR^N$ is continuous and differentiable
\item solve ($\ast$) by Newton method etc.

\bigskip
\item question: where did the FEM stiffness matrix go?
\end{itemize}
\end{frame}
