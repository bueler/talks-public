...
\item the function $\bF:\RR^N \to \RR^N$ is called the  \emph{residual}
    ...
    \end{itemize}
\item solve ($\ast$) by Newton method:
\begin{align*}
    J_\bF(\bu_k) \bp &= - \bF(\bu_k)  \\
    \bu_{k+1} &= \bu_k + \alpha_k \bp
\end{align*}

\vspace{-2mm}
    \begin{itemize}
    \item[$\circ$] Newton method should be implemented with line search to get $\alpha_k$ using merit function like $\Phi(\bu)=\frac{1}{2}\|\bF(\bu)\|^2$
    \item[$\circ$] Newton method converges in one step if $a=a(x,y)$ and $f=f(x,y)$ \dots i.e.~in the linear case
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item the Newton step $J_\bF(\bu_k) \bp = - \bF(\bu_k)$ is a linear system for $p$:
    $$A \bp = \bb$$

\vspace{-3mm}
    \begin{itemize}
    \item[$\circ$] if PDE is well-posed then $A=J_\bF(\bu_k)$ is invertible
    \item[$\circ$] often $A=J_\bF(\bu_k)$ is symmetric positive-definite
...
\begin{itemize}
\item the Newton step is $A \bp = \bb$ where $A=J_\bF(\bu_k)$ is sparse and $N\times N$
\item $O(N^3)$ dense algorithms works on small examples but struggle for big $N$
    \begin{itemize}
    \item[$\circ$] Gaussian elimination (LU decomposition)
    \item[$\circ$] Cholesky decomposition
    \end{itemize}
\item modern expectation: solve $A\bp=\bb$ by preconditioned Krylov method
    \begin{itemize}
    \item[$\circ$] define \emph{Krylov space} as $\left<\bb,A\bb,A^2\bb,\dots,A^{k-1}\bb\right>$
    \item[$\circ$] CG and GMRES are best-known Krylov methods
    \item[$\circ$] a \emph{preconditioner} is a way of rapidly solving $M\bp=\bc$ where $M$ is spectrally-equivalent to $A$
    \end{itemize}
\end{itemize}
\end{frame}
...
\item application packages (I don't know much about these!):
    \begin{itemize}
    \item[$\circ$] COMSOL Multiphysics  \hfill $\leftarrow$ grew out of Matlab toolbox
    \item[$\circ$] ABACUS
