...
\item $s(t,x,y)=$ top surface elevation
\item $h(t,x,y)=$ ice thickness
\item ${\bf U}={\bf U}(t,x,y,z)=$ horizontal velocity field
\item $a(t,x,y,z)=$ yearly-average mass balance \onslide<2>{\color{dark green} [Ed: explain m.b.]}
\end{itemize}

\begin{alertblock}<3>{key idea: \alert{ice surface is always above the bedrock}}
\end{alertblock}
\end{frame}
...
\begin{itemize}
\item FIXME: say somethin
\end{itemize}
\end{column}
\begin{column}{0.55\textwidth}
\begin{center}
\includegraphics[width=0.75\textwidth]{schoof-sliders}
\end{center}

\includegraphics[width=1.1\textwidth]{schoof-planform}
\end{column}
\end{columns}
\end{frame}
