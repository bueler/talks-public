...
\begin{frame}{nonlinear PDEs produce sparse systems too}
\begin{itemize}
\item FD$|$EM applied to a nonlinear elliptic PDE BVP gives a nonlinear (algebraic) system of $N$ equations in $N$ unknowns:
    $$\mathbf{F}(\bu)=0$$
     \vspace{-4mm}
	\begin{itemize}
	\item[$\circ$] $\mathbf{F}:\RR^N\to\RR^N$
	\item[$\circ$] $\mathbf{F}(\bw)$ is the \emph{residual} if $\bw$ is a guess at the solution $\bu$
	\end{itemize}

\item the nonlinear MSE produces a sparse linear system at each Newton step
...
\bigskip
\begin{center}
\includegraphics[width=0.3\textwidth]{figs/spybanded}
...
an algorithm which solves $\mathbf{F}(\mathbf{u}) = 0$ is \alert{\emph{optimal}} if it does $O(N)$ work
\end{minipage}
...
in the definition ``an algorithm which solves $\mathbf{F}(\mathbf{u}) = 0$ is \emph{optimal} if it does $O(N)$ work,''
\begin{itemize}
\item  ``solves'' means that it generates $\mathbf{u}_n$ so that $\frac{\|\mathbf{F}(\mathbf{u}_n)\|}{\|\mathbf{F}(\mathbf{u}_0)\|} \le \text{tol}$ where $\mathbf{u}_0$ is an initial guess
    ...
    \item[$\circ$] ``$O(\dots)$'' hides a constant which may depend on tol but may not depend on $N$
	\end{itemize}
...
\begin{frame}{sparse matrices have $O(N)$ mat-vec}
...
\begin{itemize}
\item among numerical analysts of the 1960s and 1970s, observing that sparse mat-vecs and residual evaluations are $O(N)$ for FDM and FEM discretizations of a PDE was $\text{A New Hope}^{\includegraphics[width=0.06\textwidth]{figs/anewhope}}$
\item \begin{minipage}[t]{0.7\textwidth}
naval engineer A.~Krylov (1931) observed that the solution to $A\bu=\bb$ might be well-approximated by a vector in
...
\end{minipage}
    ...
    \vspace{2mm}
	\begin{itemize}
	\item[$\circ$] computing $\bv \in \mathcal{K}_m(A,\bb)$ is an $O(mN)$ if $A\bw$ is $O(N)$
...
\includegraphics<2>[width=0.6\textwidth]{figs/NPplaneweakstrong}
\end{center}
\end{frame}
...
\bigskip
\begin{quote}
\alert{you should try to solve $N$ equations in $O(N)$ time}
\end{quote}

\bigskip
\item this is a good goal for PDE problems, but you must exploit
    \begin{itemize}
    \item[$\circ$] the locality (sparsity) of the problem, and
    \item[$\circ$] correlation (smoothness) of the solution
    \end{itemize}
\item multigrid does that!
\item for general linear algebra on dense systems $A\bu=\bb$ the goal is $O(N^2)$ \dots but how to get there is an open problem
\end{itemize}
\end{frame}
