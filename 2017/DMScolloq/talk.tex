...
\item[\textbf{1.}] \emph{Poisson equation} with Dirichlet boundary conditions:
	...
	\item[$\circ$] will use various domains $\Omega$
		\begin{itemize}
        \item including a cube
        \item and a snowflake fractal \hfill\begin{tikzpicture}[scale=1.0,baseline]
...
\item for a majority of this talk I'll use \emph{structured grids}
	\begin{itemize}
	\item[$\circ$] always when domain is $\Omega=[0,1]^d$
	\end{itemize}
\item sequence of grids $\Omega^{(k)}$ with increasing resolution $h_k \to 0$
	\begin{itemize}
	\item[$\circ$] \emph{level} $k$ equal spacing $h_k$ in each direction
    \item[$\circ$] has $N_k$ points and $N_k = O(h_k^{-d}) \to \infty$
