...
\Function{Vcycle}{$A,\bb,\bw,l$}
    ...
    \Else
        ...
        \State $\br^C = \left(\text{restriction of } \br = \bb - A \bv \text{ to } \Omega^{(l-1)}\right)$
...
\item many classical linear iterations are smoothing
	\begin{itemize}
	\item[$\circ$] e.g.~Jacobi and Gauss-Seidel
	\item[$\circ$] easy $O(N)$ operations which reduce high-frequency components of the error
	\end{itemize}
\end{itemize}
