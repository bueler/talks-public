...
\begin{itemize}
\item I am writing a book called \emph{PETSc for PDEs}
...
\item PETSc = Portable, Extensible Toolkit for Scientific computation
	...
	\begin{itemize}
	\footnotesize
	\item[$\circ$] \href{https://www.mcs.anl.gov/petsc/}{\texttt{www.mcs.anl.gov/petsc}}
	\item[$\circ$] \dots a big (ugly?) library of C codes
	\end{itemize}

\smallskip
\normalsize
\item this talk attempts to explain why anyone would even want to use a big, complicated library of C codes to solve PDEs
\end{itemize}
\end{frame}
...
\item \emph{definition}. given invertible $M$,
	...
	\end{itemize}
\item when you apply a Krylov method to these preconditioned systems it is $\kappa_2(M^{-1}A)$ or $\kappa_2(AM^{-1})$ which determines iterations

\vspace{5mm}
\item \emph{new goal}:  find preconditioners $M$ which make the condition number $\kappa=\kappa_2(M^{-1}A)$ bounded independent of $N$
\end{itemize}
\end{frame}
