...
\item what about multigrid on unstructured grids?
    \begin{itemize}
    \item[$\circ$] remember the snowflake fractal!
...
\begin{frame}{algebraic multigrid: evidence of optimality}
\begin{itemize}
...
\end{minipage}
\item now we generate level 5,6,7,8,9 approximations of the Koch fractal, and mesh them
...
\item apparently, in 1980 or so optimism about multigrid was limited to one person, Achi Brandt,
    \begin{itemize}
...
\begin{frame}{contrast: numerical convergence, and spectral methods}
\begin{itemize}
\item for a PDE BVP, as $h\to 0$ and $N\to\infty$ we \alert{first want convergence to the continuum solution}
    ...
    \end{itemize}
\item spectral methods get very close to the continuum solution for very small $N$
    \begin{itemize}
    \item[$\circ$] \emph{if} the geometry is simple (rectangle)
    \item[$\circ$] \emph{and} the solution is smooth
    \end{itemize}
\item with such a spectral method, even $O(N^3)$ solution of the discrete equations is often acceptable
...
\begin{itemize}
\item for all DE problems, need to define $N$, the number of discrete degrees of freedom, before we can talk optimality
\only<1>{
\item ODE IVP
    \begin{itemize}
...
\item ODE BVP
    \begin{itemize}
    \item[$\circ$] \FM generates tridiagonal system, so usually methods are optimal
    \end{itemize}
}
...
hyperbolic PDE IBVP
    ...
    \item[$\circ$] if we define $N = \frac{T}{\Delta t}\,\frac{L}{\Delta x}$, all methods are already optimal
    \end{itemize}
\end{minipage} \quad
\begin{minipage}[t]{35mm}
\vspace{0mm}
...
\item PDE BVP \hfill \alert{optimal requires work}
\item other PDE IBVP \hfill \alert{optimal requires work}
    \begin{itemize}
    \item[$\circ$] for example, advection-diffusion-reaction equation with reaction: $u_t + a(u)\cdot \grad u = \Div(D(u) \grad u) + f(u)$
    \end{itemize}
}
\end{itemize}
\end{frame}
...
\item sometimes you have a big machine with $\hat P$ processors 
    \begin{itemize}
    \item<1>[$\circ$] everything so far has been serial ($P=1$)
    \item[$\circ$] on graph below, runtime is third axis
    \item<3>[$\circ$] algorithms that show weak/strong scaling are often not optimal
    \end{itemize}
\end{itemize}
