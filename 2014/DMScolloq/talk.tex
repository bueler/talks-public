...
\emph{Theorem}.  There is $x\in K$ so that
   $$F(x) \cdot (y-x) \ge 0 \quad \text{ for all } x \in K.$$

\begin{columns}
\begin{column}{0.6\textwidth}
\emph{Proof}. The map $x' \mapsto \Pi (x'-F(x'))$, as a map $K\to K$, has a fixed point $x$.  It solves
  $$x=\Pi (x-F(x))=\Pi z.$$

\medskip
So $x=\Pi z$ is a projection!  But
  $$\left(\Pi z - z\right) \cdot \left(y - \Pi z\right) \ge 0$$
for all $y\in K$.

\medskip
This simplifies to $F(x) \cdot (y-x) \ge 0$.
\end{column}
\begin{column}{0.43\textwidth}
\includegraphics[width=\textwidth]{Fx-map-on-K}
\end{column}
\end{columns}
\end{frame}
