...
\begin{definition} for the linear system $A\bu=\bb$ and an invertible matrix $M\in \RR^{N\times N}$,
...
\bigskip
\begin{itemize}
\item $\alpha\in\RR$ is simply a tuning parameter, with default $\alpha=1$
\item \emph{Richardson iteration} \quad $\bv_{k+1} = \bv_k + \alpha\, \br(\bv_k)$ \quad is the $M=I$ case
	\begin{itemize}
	\item[$\circ$] $O(N)$ is clear

\medskip
	\item[$\circ$] observation: Richardson iteration for the Poisson equation \emph{is} gradient descent for the functional \quad $\displaystyle I[v] = \alpha \int_\Omega \frac{1}{2} |\grad v|^2 - f v\,dx\,dy$
	\end{itemize}
...
\item ``multiplication by $M^{-1}$ is $O(N)$'' for a fast preconditioner, but (often) not if you actually form the matrix $M^{-1}$!
\item warning: a preconditioner $M$ can be fast without being a useful tool!
\item preconditioning is an apparently simple idea, but in 21st century one sees them used as tools all over the space of solvers
\end{itemize}
\end{frame}
...
\begin{itemize}
\item we are close to multigrid, but we need to talk about multiple grids!
\item 
\end{itemize}
\end{frame}
